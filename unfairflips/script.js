...
async function loadLeaderboard() {
    try {
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            const cleanName = censorName(data.name); // Použití cenzury při výpisu
...
document.getElementById('save-btn').addEventListener('click', async () => {
    ...
    try {
        // Uložíme neupravené jméno do databáze
        await addDoc(collection(db, "scores"), { name: name, time: time, date: Date.now() });
        location.reload();
    } catch (e) {
